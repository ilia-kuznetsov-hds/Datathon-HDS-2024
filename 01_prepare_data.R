...
source_file <- here("data", "epiwatch-latest.csv")

if (!file.exists(source_file)) {
  stop("Missing source file: ", source_file)
}

# Read the CSV file from the local data export.
original_file <- read.csv(source_file, na.strings = c("", "NA"))

write.csv(original_file, here("data", "original_data.csv"), row.names = FALSE)
