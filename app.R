...
invisible(lapply(unique(packages), function(pkg) {
...
# Step 2. Build the UI elements for the dashboard
...
ui <- dashboardPage(
  ...
  dashboardBody(
    # Fix the sidebar date picker.
    # The month/year row was hidden.
    # Without it, users cannot change month.
    # These CSS rules show those controls again.
    tags$head(
      tags$style(HTML("
        .datepicker {
...
server <- function(input, output, session) {
  ...
  # TAB 4: Facet plot for trends in daily disease counts by country
...
# Run the application
# Note: The `shinyApp` function is used to create a 
# Shiny application object from the UI and server components defined above. 
# When this line is executed, it will launch the Shiny app in a web browser, 
# allowing users to interact with the dashboard.
# It works in RStudio by pressing run. 
# In VS Code, you can run this file using the command 
# `shiny::runApp("shiny_dashboard_2.R", host = "127.0.0.1", port = 3838)` in the terminal.
